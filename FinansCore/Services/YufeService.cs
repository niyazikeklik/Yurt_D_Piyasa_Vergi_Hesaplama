...
public static class YufeService
{
    ...
    private static string YufeHtml
    {
        get
        {
            if (_yufeHtml == null)
            {
                _yufeHtml = GetYufeHtml();
            }
            return _yufeHtml;
        }
    }
    private static string GetYufeHtml()
    {
        using (var client = new HttpClient())
        {
            try
            {
                HttpResponseMessage response = client.GetAsync("https://www.hakedis.org/endeksler/yi-ufe-yurtici-uretici-fiyat-endeksi").Result;
